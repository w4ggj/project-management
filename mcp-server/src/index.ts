...
function err(msg: string) {
  return { content: [{ type: 'text' as const, text: sanitize(`Error: ${msg}`) }] }
}
...
server.tool(
  'list_projects',
  'List all projects with their status, deadline, and todo counts',
  {},
  async () => {
...
server.tool(
  'get_project',
  'Get full details for a specific project including todos, notes, services, and paths',
...
server.tool(
  'update_todo',
  'Update a todo item (mark done, change text, change due date)',
  {
...
server.tool(
  'update_left_off',
  'Update the "where I left off" field for a project — shortcut for quickly noting current context',
  {
    id: z.string().describe('Project ID'),
